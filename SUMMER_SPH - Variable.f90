...
!--------------------------Tree building and grav interaction------------------------------
  recursive subroutine build_tree(node, depth, max_particles)
    ...
    do i = 1, np
      ...
      len_array(i) = node%particles(i)%s_length
    end do
...
!--------------------------SPH and Density finding subroutines---------------------------------
  ...
  recursive subroutine SPH_tree_search(node, body, bodies)
    ...
    ! is a candidate neighbor for 'body'.
      ...
      call lookup_kernel(dr, node%particles(1)%s_length, Wi, dWi_mag)
      ...
      avg_len = (body%s_length + node%particles(1)%s_length)/2
  ...
  recursive subroutine density_tree_search(node, body)
    ...
    if (node%n_particles > 1 .and. all(abs(over_dr) < (2.0_dp*node%max_len + node%size/2.0_dp)) .and. has_children) then
      ...
      dr = sqrt(sum(nr**2))
      avg_len = (node%particles(1)%s_length + body%s_length)/2
