...
!--------------------------Tree building and grav interaction------------------------------
  recursive subroutine build_tree(node, bodies,depth, max_particles)
    implicit none
    ...
    ! array to find max len
    real(dp), allocatable :: len_array(:)
    ...
    np = size(node%numbers)
    allocate(len_array(np))

    do i = 1, np
      node%mass_total  = node%mass_total  + bodies(node%numbers(i))%mass
      node%mass_center = node%mass_center + bodies(node%numbers(i))%mass * bodies(node%numbers(i))%position
      len_array(i) = bodies(node%numbers(i))%s_length
    end do

    node%max_len = maxval(len_array)
    deallocate(len_array)
    ...
    !-----------------------------
    if (np <= max_particles .or. depth == 0) return
    ...
    do p = 1, np
      child_index = 1
      do j = 1, 3
        if (bodies(node%numbers(p))%position(j) > node%center(j)) then
    ...
    do i = 1, 8
      ...
      if (counts(i) > 0) then
        allocate(node%children(i)%numbers(counts(i)))
      end if
    end do
    ...
    do p = 1, np
      ...
      node%children(child_index)%numbers(write_pos(child_index)) = node%numbers(p)
    end do
    ...
    do i = 1, 8
      if (node%children(i)%n_particles > 0) then
        call build_tree(node%children(i), bodies, depth - 1, max_particles)
      end if
    end do
  ...
  recursive subroutine particle_gravforce_one(node, p, theta)
    ...
    if ((node%size / dist) < theta .or. .not. allocated(node%children)) then
      if (node%mass_total > 0.0_dp .and. dist > 0.0_dp) then
        call lookup_grav_kernel(dist, p%s_length, W)
...
!--------------------------SPH and Density finding subroutines---------------------------------
  ...
  subroutine get_SPH(root, body)
    ...
    do i = 1, size(body)
      body(i)%alpha_rate = max(body(i)%alpha_rate / body(i)%density, 0.0_dp) + 0.15*((0.1_dp - body(i)%alpha) * body(i)%sound_speed / body(i)%s_length)
    end do
  ...
  recursive subroutine SPH_tree_search(node, body, bodies)
    ...
    ! is a candidate neighbor for 'body'.
      ...
      if (node%numbers(1) >= body%number) return
      num = node%numbers(1)
      ...
      avg_len = (body%s_length + bodies(num)%s_length)/2
  ...
  subroutine get_density(root, body)
    ...
    do i = 1, size(body)
      ...
      body(i)%omega = 0.0_dp
      !search the tree to find all neighbors and accumulate density contributions forbody(i)
      call density_tree_search(root, body(i), body)
  ...
  recursive subroutine density_tree_search(node, body, bodies)
    ...
    real(dp) :: Wj, W_h, dr, mj, dWj_mag, avg_len
    integer :: j, num
    logical :: has_children
    ...
    if (node%n_particles > 1 .and. all(abs(over_dr) < (2.0_dp*node%max_len + node%size/2.0_dp)) .and. has_children) then
      do j = 1, size(node%children)
        ! Only recurse if the child node actually contains particles
        if (node%children(j)%n_particles > 0) then
          call density_tree_search(node%children(j), body, bodies)
        end if
      end do
      ...
      nr = (body%position - bodies(num)%position) 
      dr = sqrt(sum(nr**2))
      avg_len = (bodies(num)%s_length + body%s_length)/2

      ! Lookup kernel (W) value for the calculated distance 'dr'.
      call lookup_kernel(dr, body%s_length, Wj, dWj_mag)
      W_h = - (dr*dWj_mag - 3*Wj)/body%s_length !derivative \partial_{h}W
    ...
    do i = 1, size(bodies)
      ...
      if (bodies(i)%s_length < max_length .and. bodies(i)%s_length > 0.01) then
        do while (((bodies(i)%s_length - old_len)/old_len) > convergence_criteria .and. (bodies(i)%s_length < 10.0_dp))
          ...
          call density_tree_search(root, bodies(i), bodies)

          bodies(i)%omega = 1.0_dp + (bodies(i)%s_length/(3*bodies(i)%density)) *bodies(i)%omega


          bodies(i)%s_length = bodies(i)%s_length* (1 + ((bodies(i)%mass*((eta/bodies(i)%s_length)**3))/bodies(i)%density - 1)/(3*bodies(i)%omega)) 
        end do
      else
        bodies(i)%s_length = old_len
      end if
    end do 
    !$OMP END PARALLEL DO
...
!------------------Sink and particle deletion subroutines-----------------
  ...
  subroutine initiate_sink_accretion(sinks, bodies, root)
    ...
    do i = 1, size(sinks)
      ...
      call sink2gasdists(sinks(i), bodies, root, keep_mask(i,:))
  ...
  recursive subroutine sink2gasdists(sink_i, bodies, node, mask)
    implicit none
    ...
    if (node%n_particles > 1 .and. all(abs(over_dr) < (sink_i%radius + node%size/2.0_dp)) .and. allocated(node%children)) then
      do j = 1, size(node%children)
        if (node%children(j)%n_particles > 0) call sink2gasdists(sink_i, bodies,node%children(j), mask)
      end do
      return
    
    !base case of not needing to recurse
    else if (node%n_particles == 1 .and. all(abs(over_dr) < (sink_i%radius + node%size/2.0_dp))) then
      dr = sum(sqrt((bodies(node%numbers(1))%position -  sink_i%position)**2))
      if (dr < sink_i%radius) then
        mask(node%numbers(1)) = .false.
      end if
      return
    end if
    return
...
!------------------File Reading and writing---------------------------------
  ...
  subroutine make_save(bodies, sinks ,number)
    ...
    write(io, *) 'x  ','y  ','z  ','vx  ','vy ','vz ','energy ','mass  ','alpha  ', 'smoothing'
    do i = 1, size(bodies)
      write(io, *)  bodies(i)%position(1),bodies(i)%position(2),bodies(i)%position(3), &
      bodies(i)%velocity(1),bodies(i)%velocity(2),bodies(i)%velocity(3), &
      bodies(i)%internal_energy, bodies(i)%mass, bodies(i)%alpha, bodies(i)%s_length
    end do 
    do i = 1, size(sinks)
...
!--------------------grouping subroutines-----------------------------
  ...
  subroutine create_tree(root, bodies, max_depth)
    ...
    allocate(root%numbers(size(bodies)))
    root%numbers = [(bodies(i)%number, i=1,size(bodies))]
    !allocate(root%(size(bodies))) ! Allocate space for copies of all particles in the root.
    ...
    call build_tree(root, bodies, max_depth, 1)
